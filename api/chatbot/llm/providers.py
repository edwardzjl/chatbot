...
async def get_model_info(base_url: str, model_name: str) -> dict[str, Any]:
    ...
    Raises:
        ClientResponseError: If any error occurs while making the HTTP request to fetch model information.
    ...
    try:
        ...
        async with AsyncCachedSession(
            ...
            match owned_by := model_info.get("owned_by"):
                case "vllm" | "llamacpp":
                    return model_info
                case "sglang":
                    logger.debug("Patching model info for SGLang")
                ...
                case _:
                    ...
                    logger.warning("Unknown owner '%s', assuming TGI", owned_by)
                    logger.debug("Patching model info for TGI")
                    async with await session.get("/info") as response:
                        info: dict = await response.json()
                    if info.get("model_id") == owned_by:
                        model_info["info"] = info
                        return model_info
                    logger.warning("Model '%s' not found in %s", model_name, base_url)
                    return {}
...
async def get_truncation_config(
...
async def get_max_tokens(base_url: str, model_name: str) -> int | None:
    ...
    model_info = await get_model_info(base_url, model_name)
...
async def get_num_tokens_func(
    ...
    Raises:
        ClientResponseError: If there is an error in retrieving model info.
    """
    model_info = await get_model_info(base_url, model_name)

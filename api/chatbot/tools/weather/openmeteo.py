...
hourly_params = {
    ...
    "precipitation": "Total precipitation (rain, showers, snow)",  # modified
    "precipitation_probability": "Probability of precipitation with more than 0.1 mm.",  # modified
    "weather_code": "Weather condition.",  # modified
    "visibility": "Viewing distance in meters.",  # modified
}
...
class WeatherInput(BaseModel):
    ...
    current: list[str] | None = Field(
        description="A list of weather variables to get current conditions. Available params: same as hourly params.",
        default=None,
    )
    daily: list[str] | None = Field(
        description=f"A list of daily weather variable aggregations which should be returned. Available params: {daily_params}",
        default=None,
    )
    hourly: list[str] | None = Field(
        description=f"A list of weather variables which should be returned. Available params: {hourly_params}",
        default=None,
    )
    ...
    @field_validator("hourly", "current", mode="after")
    @classmethod
    def validate_hourly_params(cls, value: list[str]) -> list[str]:
        unique_values = set(value)
        invalid_params = unique_values - hourly_params.keys()
        if invalid_params:
            raise ValueError(f"Invalid parameters: [{invalid_params}]")
        return value

    @field_validator("daily", mode="after")
    @classmethod
    def validate_daily_params(cls, value: list[str]) -> list[str]:
        unique_values = set(value)
        invalid_params = unique_values - daily_params.keys()
        if invalid_params:
            raise ValueError(f"Invalid parameters: [{invalid_params}]")
        return value
